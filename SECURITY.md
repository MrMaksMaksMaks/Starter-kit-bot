...
The project does not currently claim to be provider-agnostic — the working code (message-vs-full-transaction signing, `X-Wallet-Auth` JWT construction, payload encoding, API version differences) is Openfort-specific. What the project does commit to: isolating that specificity behind a clear internal boundary (see Roadmap, M6 — Wallet provider abstraction boundary), and documenting which parts of the security model are Openfort-specific versus generic to any backend-wallet provider. That same M6 work also includes a conceptual comparison against alternative Solana wallet/signing infrastructure, such as Turnkey and Crossmint.
